...
def send_welcome(message):
    ...
    markup.add(types.InlineKeyboardButton(
        text='Booking Appointment', callback_data='action'))
...
def vac_callback(query):
    ...
    bot.answer_callback_query(
        callback_query_id=query.id, text='You selected Booking Appointment')
    location_text = "Please choose your desire VAC."
    markup = types.InlineKeyboardMarkup(
        row_width=2)  # Two inline button in one row
    markup.add(types.InlineKeyboardButton(text='Ankara', callback_data='ankara'),
               types.InlineKeyboardButton(text='Istanbul', callback_data='istanbul'))
    bot.send_message(chat_id=query.message.chat.id, text=location_text,
                     reply_markup=markup, parse_mode='Markdown')
...
def ankara_callback(query):
    ...
    bot.answer_callback_query(
        callback_query_id=query.id, text='You selected Ankara')
...
def istanbul_callback(query):
    ...
    bot.answer_callback_query(
        callback_query_id=query.id, text='You selected Istanbul')
    user = User(query.message.chat.id)
...
def login_callback(message):
    ...
    msg = bot.send_message(chat_id=message.chat.id,
                           text=login_info_text, parse_mode='Markdown')
    time.sleep(10)
...
def login_info_checker(message):
    ...
    if len(login_data) < 3:
        ...
        if gmail == "gmail":
            ...
            markup.add(types.InlineKeyboardButton(text='Yes', callback_data='otp_yes'),
                       types.InlineKeyboardButton(text='No', callback_data='otp_no'))
            bot.send_message(chat_id=message.chat.id, text=otp_method,
                             reply_markup=markup, parse_mode='Markdown')
        else:
            global otp_automate
...
def gmail1_callback(query):
    ...
    bot.send_message(chat_id=query.message.chat.id,
                     text=password_text, parse_mode='Markdown')
...
def gmail2_callback(query):
    ...
    otp_automate = False
    bot.send_message(chat_id=query.message.chat.id,
                     text="Trying for login...", parse_mode='Markdown')
    ...
    while status:  # Loop for login
        try:
            ...
            driver.find_element_by_class_name(
                "inactive-link")  # Bot successfully logged in
            bot.send_message(chat_id=message.chat.id,
                             text="Successful Login...", parse_mode='Markdown')
            status = False
        except:
            try:  # Here, the account was locked
                login_error = BeautifulSoup(driver.page_source, 'html.parser')
                # check if there is an error with bs4
                login_error.find_all(
                    'div', attrs={'class': 'validation-summary-errors'})
                ...
                bot.send_message(
                    chat_id=message.chat.id, text="Error in Login. Trying again...", parse_mode='Markdown')
                time.sleep(5)
                get_captcha(message)   # request captcha again
                return
...
def otp(message):
    ...
    if not otp_automate:
        ...
        bot.send_message(chat_id=message.chat.id,
                         text="Getting OTP...", parse_mode='Markdown')
        user = all_users[message.chat.id]
        account_email = user.email
        ...
        try:   # Some unindendent errors!!
            ...
            opt_error.find_all(
                'div', attrs={'style': 'color: red; padding: 20px;'})
            bot.send_message(
                chat_id=message.chat.id, text="Error in OTP! Regenerating...", parse_mode='Markdown')
            driver.find_element_by_id("txtbox").click()  # Re-generate OTP
            ...
            while (end - start) > 160:
                ...
                time.sleep(90)
                start = time.time()
                # Calls gmail_check function for automatic checking
                user_opt = gmail_ckeck(account_email, email_password)
                end = time.time()
            ...
            try:   # We passed successfully OTP step
                driver.find_element_by_xpath(
                    "//*[contains(text(), '15')]").click()
                text = "I reached Calendar. Please insert your date like example below./n/nExample: *2020-09-25*."
                msg = bot.send_message(
                    chat_id=message.chat.id, text=text, parse_mode='Markdown')   # Gets desired date
                time.sleep(10)
...
def select_day(message):
    ...
    try:
        ...
        hours = soup_level.find_all(
            'input', attrs={'name': 'selectedTimeBand'})
        bot.send_message(chat_id=message.chat.id,
                         text=hours, parse_mode='Markdown')
    except:
        bot.send_message(chat_id=message.chat.id,
                         text="Sorry...Date is not available", parse_mode='Markdown')
